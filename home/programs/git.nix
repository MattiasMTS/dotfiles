{ username, ... }:
{
  ...
  settings = {
    ...
    alias = {
      standup = "!git log --since=yesterday --author=\"$(git config user.name)\" --all --format='%h %s (%ar)' --shortstat";
      weekly = "!git log --since='1 week ago' --author=\"$(git config user.name)\" --all --format='%h %s (%ar)' --shortstat";
